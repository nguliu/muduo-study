...
class Inspector : boost::noncopyable
{
 ...
 private:
  typedef std::map<string, Callback> CommandList;  //command到对应callback的映射
  typedef std::map<string, string> HelpList;		   //command到对应helping的映射

  void start();
  void onRequest(const HttpRequest& req, HttpResponse* resp); //注册到HttpServer处理正确的请求

  HttpServer server_;
  boost::scoped_ptr<ProcessInspector> processInspector_;
  MutexLock mutex_;
  std::map<string, CommandList> commands_;  //module到command-callback list的映射
  std::map<string, HelpList> helps_;		//module到command-helping list的映射
};
